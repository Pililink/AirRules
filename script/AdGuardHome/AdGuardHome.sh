...
interface='br-lan'
new_answer=$(ip addr show $interface | grep 'inet ' | awk '{print $2}' | cut -d'/' -f1)
...
set_domain(){
    if [ -e "$ad_guard_home_dir/yq/yq" ]; then
        ...
        if [ -z "$yq_result" ]; then
            ...
            "$ad_guard_home_dir/yq/yq" ea '.filtering.rewrites += [{ "domain": "'"$domain"'", "answer": "'"$new_answer"'" }]' -i "$config_file"
        else
            # 记录存在，更新 answer 值
            echo "已找到域名 $domain 的记录，更新 answer 值为 $new_answer..."
            "$ad_guard_home_dir/yq/yq" ea '(.filtering.rewrites[] | select(.domain == "'"$domain"'")).answer = "'"$new_answer"'"' -i "$config_file"
        fi
    else
        echo "文件不存在"
        install_yq
